...
mod tests {
    ...
    fn test_ray_creation_and_at() {
        ...
        let direction = vec3(0.0, 1.0, 0.0); // direction is along the y-axis
        let ray = Ray::new(origin, direction);

        // Check that the ray's origin is correctly set
        assert_eq!(ray.origin, origin);
        // Check that the direction is normalized
        assert_eq!(ray.direction, vec3(0.0, 1.0, 0.0)); // already normalized
        ...
        assert_eq!(point_at_t_5, vec3(1.0, 7.0, 3.0)); // origin + 5 * direction
    }
    ...
    fn test_zero_direction_normalization() {
        ...
        let direction = vec3(0.0, 0.0, 0.0); // zero direction vector
        let ray = Ray::new(origin, direction);

        assert_eq!(ray.direction, vec3(0.0, 0.0, 0.0));
    }
    ...
    fn test_ray_at_different_times() {
        ...
        let origin = vec3(1.0, 2.0, 3.0);
        let direction = vec3(1.0, 0.0, 0.0); // arbitrary direction
        let ray = Ray::new(origin, direction);

        // Test `at` at different `t` values
        let point_at_t_1 = ray.at(1.0);
        assert_eq!(point_at_t_1, vec3(2.0, 2.0, 3.0)); // origin + direction

        let point_at_t_2 = ray.at(2.0);
        assert_eq!(point_at_t_2, vec3(3.0, 2.0, 3.0)); // origin + 2 * direction
    }
}